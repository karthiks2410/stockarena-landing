...
export const AnimatedThemeToggle = ({
  ...
  return (
    <Button
      onClick={toggleTheme}
      className={cn(
        "px-2.5 rounded-full border-gray-300 dark:border-gray-600 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm hover:border-emerald-400 dark:hover:border-emerald-400 hover:bg-white/90 dark:hover:bg-gray-800/90",
        className
      )}
      variant="outline"
...
const SolarSwitch = ({ isDark }: { isDark: boolean }) => {
  ...
  // Update motion values when isDark changes
  useEffect(() => {
